    ...
    def wait_for_service_ready(self, timeout: int = DEFAULT_MAX_WAIT_TIME, 
        ...
        for i in range(max_checks):
            if self.server_process and self.server_process.poll() is not None:
                ...
                raise RuntimeError(f"Service process exited early (rc={rc}). See service logs above.")
...
class VLLMServiceManager(BaseServiceManager):
    ...
    def _build_start_command(self, port: int) -> List[str]:
        ...
        cmd = [
            ...
            "--served-model-name", str(self.config.model), 
            "--port", str(port),
        ...
        if (not cpu_only) and accelerator == "nvidia":
            ...
            gmu = framework_kwargs.get("gpu_memory_utilization", DEFAULT_VLLM_GPU_MEMORY_UTILIZATION)
            ...
            logger.warning(f"vLLM accelerator='{accelerator}', cpu_only={cpu_only}; may not be fully supported.")
