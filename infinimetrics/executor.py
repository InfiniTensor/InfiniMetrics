...
class Executor:
    ...
    def _build_error_response(self, error_msg: str, result_code: int) -> Dict[str, Any]:
        ...
        # Remove injected fields (_testcase, _run_id, _time) to keep only user-provided config
        cleaned_config = {}
        for key, value in config.items():
            if not key.startswith("_"):  # Skip _testcase, _run_id, _time (injected metadata)
                cleaned_config[key] = value

        # Try to extract device information from config
        device_used = 0
        gpus_per_node = 0
        nodes = 1

        # Try to get device_used from config
        ...
        # Try to get nodes from config (for multi-node)
