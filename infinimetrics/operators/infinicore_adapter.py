    ...
    def _handle_flops(self, metric: dict, context: dict, config: dict = None):
        ...
        if context.get("latency_ms") and context.get("latency_ms", 0) > 0:
            ...
            if flops > 0 and latency_sec > 0:
                ...
                # Use scientific notation for very small values, otherwise round to 4 decimals
                if tflops < 0.0001:
                    metric.update({
                        "value": tflops,  # Keep full precision
                        "type": "scalar",
                        "raw_data_url": "",
                        "unit": "TFLOPS"
                    })
    ...
    def _handle_bandwidth(self, metric: dict, context: dict, config: dict = None):
        ...
        if context.get("latency_ms") and context.get("latency_ms", 0) > 0:
            ...
            if bandwidth_info['total_bytes'] > 0 and latency_sec > 0:
                ...
                # Use scientific notation for very small values, otherwise round to 4 decimals
                if bandwidth_gbs < 0.0001:
                    metric.update({
                        "value": bandwidth_gbs,  # Keep full precision
                        "type": "scalar",
                        "raw_data_url": "",
                        "unit": "GB/s"
                    })
