        ...
        try:
            if path.is_file():
                ...
                for json_file in sorted(path.glob('*.json')):
                    ...
                    all_inputs.extend(inputs)
                logger.info(f"Loaded {len(list(path.glob('*.json')))} file(s) from {path_str}")
            else:
...
def run_tests(inputs: List[Dict[str, Any]], default_config: Dict[str, Any]) -> Dict[str, Any]:
    """Run tests using Dispatcher."""
    for test_input in inputs:
        test_input.setdefault('config', {}).setdefault('output_dir', default_config.get('output_dir', './output'))
...
def main():
    ...
    parser.add_argument('inputs', nargs='+', help='Input files or directories containing test specifications')
