...
# Page configuration
st.set_page_config(
    page_title="通信测试分析 | InfiniMetrics",
    page_icon="🔗",
    layout="wide"
)
    ...
    try:
        ...
        if run_id_kw:
            filtered_runs = [
                ...
                if run_id_kw in (r.get("run_id") or "")
            ]
        ...
        with tab1:
                ...
                if len(selected_runs) == 1:
                    ...
                    for metric in metrics:
                        if metric.get("name") == "comm.bandwidth" and metric.get("data") is not None:
                            df = metric["data"]
                            fig = plot_bandwidth_vs_size(df, f"带宽分析 - {run['operation']}", y_log_scale)
                            st.plotly_chart(fig, use_container_width=True)
                            break
                else:
                    # Multiple runs - comparison
                    fig = plot_comparison_matrix(selected_runs, "bandwidth", y_log_scale)
                    st.plotly_chart(fig, use_container_width=True)
                ...
                if len(selected_runs) == 1:
                    ...
                    for metric in metrics:
                        if metric.get("name") == "comm.latency" and metric.get("data") is not None:
                            df = metric["data"]
                            fig = plot_latency_vs_size(df, f"延迟分析 - {run['operation']}", y_log_scale)
                            st.plotly_chart(fig, use_container_width=True)
                            break
                else:
            ...
            if len(selected_runs) == 1:
                ...
                c1.metric(
                    "峰值带宽",
                    f"{core['bandwidth_gbps']:.2f} GB/s" if core["bandwidth_gbps"] else "-"
                )
                c2.metric(
                    "平均延迟",
                ...
                c3.metric(
                    "TTFT",
                    f"{core['ttft_ms']:.2f} ms" if core["ttft_ms"] else "-"
                )
        ...
        with tab2:
            ...
            for run in selected_runs:
                with st.expander(f"{run['operation']} ({run['device_used']} GPUs) - 原始数据"):
                    metrics = run.get("data", {}).get("metrics", [])
                    for metric in metrics:
        ...
        with tab3:
            ...
            for run in selected_runs:
                with st.expander(f"{run['operation']} ({run['device_used']} GPUs) - 配置详情"):
