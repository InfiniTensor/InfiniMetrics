...
class InfiniMetricsDataLoader:
    ...
    def list_test_runs(self, test_type: str = None) -> List[Dict[str, Any]]:
        ...
        for json_file in self.results_dir.rglob("*.json"):
            try:
                ...
                if "summary" in json_file.name.lower() or "dispatcher" in json_file.name.lower():
                    continue
    ...
    def load_csv_data(self, csv_url: str, json_data: Dict[str, Any], json_path: Path) -> Optional[pd.DataFrame]:
    ...
    def _extract_run_info(self, data: Dict[str, Any], json_path: Path) -> Dict[str, Any]:
        """Extract run info from test result data."""
        config = data.get("config", {})
        resolved = data.get("resolved", {})
        
        # Device used: try resolved first, then config
        device_used = resolved.get("device_used") or config.get("device_used") or config.get("device_involved", 1)
...
def load_summary_file(summary_path: str = "./summary_output") -> List[Dict[str, Any]]:
    ...
    if summary_dir.exists():
        for json_file in sorted(summary_dir.glob("dispatcher_summary_*.json"), reverse=True):
            try:
                with open(json_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
