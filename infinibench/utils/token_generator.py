...
class TokenGenerator:
    ...
    def _init_token_info(self):
        ...
        if hasattr(self.tokenizer, "vocab_size"):
            ...
            try:
                ...
                logger.warning(
                    f"Cannot determine vocab_size, using default: {self.vocab_size}"
                )
        ...
        logger.info(
            f"TokenGenerator initialized: vocab_size={self.vocab_size}, "
    ...
    def _get_valid_token_ids(self) -> List[int]:
        ...
        if not valid_ids:
            ...
            valid_ids = list(
                range(self.config.min_token_id, self.config.max_token_id + 1)
            )
    ...
    def generate_tokens(self, num_tokens: int) -> List[int]:
        ...
        logger.debug(
            f"Generated {num_tokens} random tokens from {len(self.valid_token_ids)} valid tokens"
        )
        return tokens

    def generate_token_batch(
        self, batch_size: int, tokens_per_sample: int
    ) -> List[List[int]]:
